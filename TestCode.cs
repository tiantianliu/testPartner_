...
namespace testPartner
{
    ...
    class TestCode
    {
        ...
        double change_x = 0;//x方向增量
        double change_y = 0;//y方向增量
        float r = 20;//半径
        double change = 0;//改变的角度
        ...
        public void Test1(PictureBox picbox)
        {
            //取pic作为画布尺寸
            _canvaswidth = picbox.Width;
            _canvasheight = picbox.Height;
            ...
            {
                change += 0.01;
                change_x = r * Math.Sin(Math.PI * change);
                change_y = r * Math.Cos(Math.PI * change);
                picbox.Paint += callback;
                picbox.Refresh();
                picbox.Paint -= callback;
            };
        ...
        void _draw1(System.Drawing.Graphics g)
        {
            float w = 3;
            float h = 3;
            ...
            g.DrawImage(img01, x + (float)change_x, y + (float)change_y, w, h);
